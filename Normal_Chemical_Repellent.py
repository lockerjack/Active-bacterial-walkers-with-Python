#This code is the chemotactic biasing for the system.
...
def Initialize_Walkers(N,Le):		#Initialized at the centre
...
Repellent = numpy.zeros((size,size)) + 0.0
    ...
    if t>100:
        ...
        while (i<len(actives)):
            for i in range(0,len(inactives)):
                x = int(inactives[i][0])
                y = int(inactives[i][1])
                Repellent[x][y] += Emission_rate*(1-Peptone[x][y])
